...
for iSub = 1:length(UMBatchSubjs)
  ...
  % We use the "spm_select('ExtFPList',[directory],[file wildcard])" command
  ...
  for iRun = 1:length(UMImgDIRS{iSub})
    ...
    fprintf('Normalizing a total of %d %s''s from directory %s\n',nFiles,UMVolumeWild,UMImgDIRS{iSub}{iRun});
    % 
    % The other step to apply the normalization to the SPGR.
    %
    results = UMBatchWarpSeg(ParamImage,Images2Write,UMTestFlag,VoxelSize,OutputName);      
